...
} from 'src/modules/timeline/types/effective-timeline-activity-rule.type';
...
const getJoinColumnName = (
  ...
  return settings.joinColumnName ?? undefined;
};
...
export const buildJunctionTargetShape = ({
  ...
  if (!isDefined(junctionSourceJoinColumnName)) {
    return undefined;
  }
  ...
  const junctionTargetJoinColumns = junctionTargetFlatFieldMetadatas
    .map(
      (flatFieldMetadata): TimelineActivityRuleTargetJoinColumn | undefined => {
        const joinColumnName = getJoinColumnName(flatFieldMetadata);
