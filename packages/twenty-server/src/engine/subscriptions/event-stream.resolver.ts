...
import { MetadataEventTranslationService } from 'src/engine/subscriptions/metadata-event/services/metadata-event-translation.service';
...
export class EventStreamResolver {
  constructor(
    ...
    private readonly metadataEventTranslationService: MetadataEventTranslationService,
  ) {}
  ...
  async onEventSubscription(
    ...
    try {
      ...
      iterator = mapAsyncIterator(rawIterator, async (payload) => ({
        ...payload,
        metadataEvents:
          await this.metadataEventTranslationService.translateMetadataEvents({
            metadataEvents: payload.metadataEvents,
            locale,
            workspaceId: workspace.id,
          }),
