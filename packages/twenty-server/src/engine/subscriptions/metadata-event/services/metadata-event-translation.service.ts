...
@Injectable()
export class MetadataEventTranslationService {
  constructor(
  ...
  async translateMetadataEvents({
    ...
    if (translatableEvents.length === 0) {
    ...
    const flatObjectMetadataMaps = translatableEvents.some(
    ...
    return metadataEvents.map((metadataEvent) => {
      ...
      if (!isTranslatableMetadataName(metadataName)) {
      ...
      for (const key of RECORD_KEYS) {
        ...
        const resolvedRecord = resolveMetadataEventRecordTranslations({
          metadataName,
          record,
