...
// Events are broadcast once per workspace, so a record reaches a subscriber
// still carrying its source strings and its overrides.
export const resolveMetadataEventRecordTranslations = ({
  metadataName,
  record,
  i18nContext,
}: {
  ...
  const translatableProperties = new Set<string>(
    TRANSLATABLE_PROPERTIES_BY_METADATA_NAME[metadataName],
  );
